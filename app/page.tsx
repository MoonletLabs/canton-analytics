...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    async function runSequence() {
      let didStartLiveness = false;
      let didStartOpenVotes = false;
      try {
        ...
        if (!cancelledRef.current) {
          setLoadingRound(false);
          setLoadingDso(false);
          if (didStartLiveness) setLoadingLiveness(false);
          if (didStartOpenVotes) setLoadingOpenVotes(false);
        }
      }
    }
    runSequence();
  ...
  const validatorChart = liveness.slice(0, 12).map((v) => ({
    ...
    name:
      v.validator_id.length > 20
        ? v.validator_id.slice(0, 20) + "…"
        : v.validator_id,
  ...
  const tenants: TenantRow[] = liveness.map((v, i) => ({
    ...
    slaPercent:
      v.missed_rounds === 0
        ? 100
        : Math.max(0, 100 - (v.missed_rounds / 144) * 100),
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground text-lg mt-1">
            Network metrics, validators, governance, and operator view — one
            place
          </p>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(200px,320px)_1fr] mb-8">
                ...
                {loadingRound ? (
                  ...
                  <div className="text-center">
                    ...
                    <div className="text-xs text-muted-foreground mt-1">
                      {formatRoundTs(roundInfo.timestamp)}
                    </div>
                  </div>
                ) : (
          ...
          <Card className="bg-card/80 border-border">
            <CardHeader>
              ...
              <CardDescription>
                Liveness vs missed rounds (top 12)
              </CardDescription>
            </CardHeader>
            <CardContent>
                          ...
                          return (
                            <div className="rounded-lg border bg-card p-2 shadow-lg">
                              ...
                              <div className="text-xs mt-1">
                                Liveness: {p.liveness} · Missed: {p.missed}
                              </div>
                            </div>
                          );
                        }}
              ...
              <div className="mt-4 flex flex-wrap gap-4">
                <Link href="/validators">
                  <Button variant="outline" size="sm">
                    Validators <ArrowRight className="ml-1 h-4 w-4" />
                  </Button>
                </Link>
                <Link href="/super-validators">
                  <Button variant="outline" size="sm">
                    Super Validators <ArrowRight className="ml-1 h-4 w-4" />
                  </Button>
                </Link>
              </div>
            </CardContent>
        ...
        <Card className="mb-8 bg-card/80 border-border">
          <CardHeader>
            ...
            <CardDescription>
              DSO state and open governance votes
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
                ...
                <div className="flex gap-6 flex-wrap text-sm">
                  <span className="text-muted-foreground">
                    ...
                    <strong className="text-foreground">
                      {dsoState.voting_threshold ?? 0}
                    </strong>
                  </span>
                  <span className="text-muted-foreground">
                    SV nodes:{" "}
                    <strong className="text-foreground">
                      {dsoState.sv_node_states?.length ?? 0}
                    </strong>
                  </span>
                </div>
              )
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {openVotes
                        .slice(
                          (openVotesPage - 1) * openVotesPageSize,
                          openVotesPage * openVotesPageSize
                        )
                        .map((v, i) => {
                          const voteId =
            ...
            <Link href="/governance">
              <Button variant="outline" size="sm">
                Governance <ArrowRight className="ml-1 h-4 w-4" />
              </Button>
            </Link>
          </CardContent>
        </Card>
        ...
        <Card className="mb-8 bg-card/80 border-border">
          <CardHeader>
            ...
            <CardDescription>
              Per-validator uptime from blockchain liveness
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loadingLiveness ? (
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {tenants
                        .slice(
                          (tenantsPage - 1) * tenantsPageSize,
                          tenantsPage * tenantsPageSize
                        )
                        .map((t) => (
                          <tr
                            key={t.id}
                          ...
                          >
                            ...
                            <td className="py-3 px-2 font-mono text-xs">
                              <Link
                                href={`/validators/${encodeURIComponent(t.validatorId)}`}
                                className="text-primary hover:underline"
                              >
                                {t.validatorId}
                              </Link>
        ...
        <Card className="mb-8 bg-card/80 border-border">
          <CardHeader>
            ...
            <CardDescription>
              Per-tenant reward attribution from on-chain liveness
            </CardDescription>
          </CardHeader>
          <CardContent>
            {loadingLiveness ? (
                        ...
                        .map((t) => (
                          ...
                          >
                            ...
                            <td className="py-3 px-2 text-right font-semibold">
                              {t.rewardAttribution.toLocaleString()} CC
                            </td>
                          </tr>
                        ))}
                    </tbody>
